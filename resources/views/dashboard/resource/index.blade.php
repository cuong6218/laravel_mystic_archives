                            ...
                            <tbody>
                                <?php
                                    foreach($datas as $data){
                                        ...
                                        if($enableButtons['read'] == 1){
                                            echo '<td>';
                                            echo '<a href="' . route("resource.show", ['tables' => $form->id, 'resource' => $data['id'] ] ) . '" class="btn btn-primary">Show</a>';
                                            echo '</td>';
                                        }
                                        if($enableButtons['edit'] == 1){
                                            echo '<td>';
                                            echo '<a href="' . route("resource.edit", ['tables' => $form->id, 'resource' => $data['id'] ] ) . '" class="btn btn-primary">Edit</a>';
                                            echo '</td>';
                                        }
                                        if($enableButtons['delete'] == 1){
                                            echo '<td>';
                                            echo '<form action="' . route("resource.destroy", ['tables' => $form->id, 'resource' => $data['id'] ] )  . '" method="POST">';
                                                ?>
                                                    @csrf
                                                    @method('DELETE')
                                                <?php
