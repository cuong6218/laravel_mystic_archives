...
class ResourceService{
    ...
    public function getRelations( $columns ){
        ...
        foreach($relations as $relation){
            $result['relation_' . $relation['thisTableColumn']] = DB::table($relation['tables'])->select('id', $relation['column'] . ' AS name')->get();
        }
        return $result;
    }
    ...
    public function getIndexDatas($id){
        ...
        foreach( $relations as $relation ){
            $table->leftJoin($relation['tables'], $relation['tables'] . '.id', '=', $form->table_name . '.' . $relation['thisTableColumnName']);
            if(empty($relation['column'])){
                $table->addSelect($relation['tables'] . '.name AS relation_' . $relation['thisTableColumnName']);
            }else{
                $table->addSelect($relation['tables'] . '.' . $relation['column'] . ' AS relation_' . $relation['thisTableColumnName']);
            }
        }
